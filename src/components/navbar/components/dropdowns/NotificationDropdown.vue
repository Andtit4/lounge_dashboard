...
// Fonction pour convertir les codes de locale au format attendu par RelativeTimeFormat
const formatLocale = (localeCode: string) => {
  ...
  return localeMap[localeCode] || 'en-GB'
}

// Créer une référence pour le formateur RelativeTimeFormat
const rtf = ref(new Intl.RelativeTimeFormat(formatLocale(locale.value), { style: 'short' }))

// Mettre à jour rtf lorsque la locale change
watch(locale, (newLocale) => {
  rtf.value = new Intl.RelativeTimeFormat(formatLocale(newLocale), { style: 'short' })
})
...
const notificationsWithRelativeTime = computed(() => {
  ...
  return list.map((item, index) => {
    ...
    try {
      return {
        ...item,
        updateTimestamp: rtf.value.format(-1 * Math.round(timeDifference / TIME_NAMES[timeName]), timeName),
        separator,
      }
    } catch (error) {
