...
import { CreateSubscriptionDto, SubscriptionType } from './dto/create-subscription.dto';
...
export class SubscriptionsService {
  ...
  async checkStatus(id: string): Promise<{ isActive: boolean; daysRemaining: number }> {
    const subscription = await this.findOne(id);
    const now = new Date();
  ...
  async createTransaction(createDto: CreateSubscriptionTransactionDto): Promise<SubscriptionTransaction> {
    ...
    const subscriptionType = this.normalizeSubscriptionType(transactionData.subscriptionType);
  ...
  async findTransactionsByUser(userId: string): Promise<SubscriptionTransaction[]> {
    return await this.transactionsRepository.find({
      where: { userId },
      order: { transactionDate: 'DESC' }
    });
  ...
  async updateTransaction(id: string, updateDto: UpdateSubscriptionTransactionDto): Promise<SubscriptionTransaction> {
    ...
    if (updateDto.subscriptionType) {
      updatedTransaction.subscriptionType = this.normalizeSubscriptionType(updateDto.subscriptionType);
    }
    if (updateDto.amount) {
    ...
    if (updateDto.status === 'completed' || transaction.status === 'completed') {
      const updateUserData: any = {};
      
      if (updateDto.subscriptionType) {
        updateUserData.subscriptionType = this.normalizeSubscriptionType(updateDto.subscriptionType);
      }
      if (updateDto.endDate) {
